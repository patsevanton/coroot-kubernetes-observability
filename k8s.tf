...
resource "yandex_iam_service_account" "sa_k8s_editor" {
  ...
  name      = "coroot-sa-k8s-editor" # Имя сервисного аккаунта
}
...
resource "yandex_kubernetes_cluster" "coroot" {
  name       = "coroot" # Имя кластера
  folder_id  = local.folder_id
...
# Группа узлов для Kubernetes-кластера. Coroot ставит: coroot (StatefulSet), Prometheus (Deployment),
# ClickHouse (StatefulSet, 1 shard / 1 replica) + clickhouse-keeper (StatefulSet, 3 реплики),
# coroot-node-agent (DaemonSet на каждой ноде) + coroot-cluster-agent — поэтому 3 ноды, по одной на зону.
resource "yandex_kubernetes_node_group" "k8s_node_group" {
  description = "Node group for the Managed Service for Kubernetes cluster"
  name        = "coroot-node-group"
  cluster_id  = yandex_kubernetes_cluster.coroot.id
...
provider "helm" {
  kubernetes = {
    host                   = yandex_kubernetes_cluster.coroot.master[0].external_v4_endpoint   # Адрес API Kubernetes
    cluster_ca_certificate = yandex_kubernetes_cluster.coroot.master[0].cluster_ca_certificate # CA-сертификат
    exec = {
...
# Настройка провайдера Kubernetes (для Secret с паролем администратора Coroot)
provider "kubernetes" {
  host                   = yandex_kubernetes_cluster.coroot.master[0].external_v4_endpoint
  cluster_ca_certificate = yandex_kubernetes_cluster.coroot.master[0].cluster_ca_certificate
  exec {
    api_version = "client.authentication.k8s.io/v1beta1"
...
resource "helm_release" "ingress_nginx" {
  ...
  depends_on = [
    yandex_kubernetes_cluster.coroot,
...
output "k8s_cluster_credentials_command" {
  value = "yc managed-kubernetes cluster get-credentials --id ${yandex_kubernetes_cluster.coroot.id} --external --force"
}
