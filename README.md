...
Важно понимать: «языковые профилировщики» — это **не плагин, не библиотека и не модуль в коде вашего приложения**. Это механизмы внутри самих агентов Coroot, которые обращаются к чужому процессу снаружи:

- **Go heap** — `coroot-node-agent` сам читает структуру `runtime.MemProfile` прямо из памяти процесса (`/proc/<pid>/mem`). В приложение ничего не подключается и код не меняется.
- **Go pprof** — `coroot-cluster-agent` скрейпит стандартный HTTP-эндпоинт `/debug/pprof`, который Go-рантайм отдаёт из коробки (его лишь нужно экспортировать в приложении и пометить под аннотациями).
- **Java** — `coroot-node-agent` находит HotSpot JVM и динамически подгружает в неё нативную `libasync-profiler.so` через JVM Attach API. Библиотека async-profiler приходит с агентом, а не с вашим приложением.
