...
Инспекции подсветят высокую утилизацию CPU одним подом. В колонке **CPU** — **shortage**: сколько времени процессы ждали CPU, но не получали его. Метрика — `container_resources_cpu_delay_seconds_total` (Linux delay accounting); delay 500ms/сек означает, что к каждой секунде обработки запросов добавляется 500ms задержки.
